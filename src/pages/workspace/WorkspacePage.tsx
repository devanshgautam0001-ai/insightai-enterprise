...
export const WorkspacePage: React.FC = () => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Automatically refresh the workspace list from PostgreSQL
      const list = await api.get('/api/workspaces');
      const safeList = Array.isArray(list) ? list : [];
      useUIStore.setState({ workspaces: safeList });

      // Find the created workspace in the fresh list or fallback to newWs
      const activeWs = safeList.find((w: any) => w.id === newWs?.id) || newWs;
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto animate-fade-in" id="workspace-selection-page">
      ...
      {loading && workspaces.length === 0 ? (
        ...
        <div className="text-center py-12 glass-card rounded-2xl border border-white/5 max-w-md mx-auto p-8 space-y-6" id="no-workspaces-empty-state">
          <Building className="w-12 h-12 text-zinc-600 mx-auto mb-2" />
          <div className="space-y-2">
            <h3 className="text-xl font-display font-bold text-zinc-300">No workspace found</h3>
            <p className="text-zinc-500 text-xs max-w-sm mx-auto">
              You do not have access to any secure corporate workspaces yet. Register your first workspace to begin.
            </p>
          </div>
          {!showCreate ? (
            <button
              onClick={() => setShowCreate(true)}
              className="px-6 py-2.5 rounded-xl bg-blue-500 hover:bg-blue-600 text-white font-semibold text-sm transition-all shadow-lg hover:shadow-blue-500/10 cursor-pointer"
              id="create-workspace-empty-btn"
            >
              Create Workspace
            </button>
          ) : (
            <div className="border border-white/10 bg-white/[0.01] rounded-xl p-5 text-left space-y-4 max-w-sm mx-auto">
              <form onSubmit={handleCreate} className="space-y-4">
                <div className="space-y-3">
                  <div>
                    <label className="block text-[10px] font-mono text-zinc-500 uppercase tracking-wider mb-1.5">Workspace Name</label>
                    <input
                      type="text"
                      placeholder="e.g. Alpha Operations"
                      value={newWorkspaceName}
                      onChange={(e) => setNewWorkspaceName(e.target.value)}
                      className="w-full bg-white/[0.02] border border-white/10 rounded-lg px-3 py-2 text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-blue-500"
                      required
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] font-mono text-zinc-500 uppercase tracking-wider mb-1.5">Division / Department</label>
                    <input
                      type="text"
                ...
                <div className="flex gap-2 justify-end pt-2">
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-zinc-400 text-xs font-semibold cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-3 py-1.5 rounded-lg bg-blue-500 hover:bg-blue-600 text-white text-xs font-semibold flex items-center gap-1 cursor-pointer"
                  >
              ...
              <form onSubmit={handleCreate} className="space-y-3 flex-grow flex flex-col justify-between">
                ...
                <div className="flex gap-2 justify-end">
                  <button
                    type="button"
                    onClick={() => setShowCreate(false)}
                    className="px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-zinc-400 text-[10px] font-semibold cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-3 py-1.5 rounded-lg bg-blue-500 hover:bg-blue-600 text-white text-[10px] font-semibold flex items-center gap-1 cursor-pointer"
                  >
